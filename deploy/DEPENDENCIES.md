...
|------|------------------|:---:|------|
| **CA 材料** | `squid-ca` secret（私钥+证书 bundle，SSL-Bump 签发用）。`DEPLOY.md` 手动步骤引用 `../squid-openssl/ca/006-ca-new/*.pem` | 🔴 硬阻塞 | 该 `squid-openssl/` 目录**不在本仓库**。本目录没有 CA 生成步骤。（Compose 侧 `scripts/setup.sh` 有可复用的自签 CA 逻辑） |
...
1. **CA 生成步骤**（🔴）：新增一个脚本/文档，在本目录内自签一套 CA（bundle=私钥+证书、公钥），
   产物直接喂给 `DEPLOY.md §1.2` 的 `kubectl create secret squid-ca`。可直接借用
   Compose 侧 `scripts/setup.sh` 的 openssl 逻辑。
